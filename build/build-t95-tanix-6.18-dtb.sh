...
# The hardware wiring remains the verified Tanix reference.  Only the board
# identity is made explicit so U-Boot can select an unambiguous T95 filename.
...
unchanged=all hardware wiring, AC300 internal PHY address 0, MDIO mux, SID calibration
...
install -m 0644 "${BASH_SOURCE[0]}" "$artifact/build-recipe.sh"
{
    ...
    printf 'scope=Tanix 6.18 DTB with T95 identity only; no kernel build\n'
